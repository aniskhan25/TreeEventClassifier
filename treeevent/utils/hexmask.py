...
def get_masks(coords, train_size=0.7):
    ...
    train_indices, test_indices = next(gs.split(indices, groups=coords.id))
    train_indices, val_indices  = next(gs.split(train_indices, groups=coords.loc[train_indices].id))
